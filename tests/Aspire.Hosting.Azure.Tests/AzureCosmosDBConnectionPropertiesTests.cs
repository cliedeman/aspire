...
public class AzureCosmosDBConnectionPropertiesTests
{
    ...
    public void AzureCosmosDbResourcePreviewEmulatorGetConnectionPropertiesReturnsExpectedValues(string? imageTag)
    {
        ...
        Assert.Collection(
            ...
            {
                Assert.Equal("Uri", property.Key);
                Assert.Equal("http://{cosmos.bindings.emulator.host}:{cosmos.bindings.emulator.port}", property.Value.ValueExpression);
            },
            property =>
            {
                Assert.Equal("AccountKey", property.Key);
            ...
            {
                Assert.Equal("ConnectionString", property.Key);
                Assert.Equal("AccountKey=C2y6yDjf5/R+ob0N8A7Cgv30VRDJIWEHLM+4QDU5DE2nQ9nDuVTqobD4b8mGGyPMbIZnqyMsEcaGQy67XIw/Jw==;AccountEndpoint=http://{cosmos.bindings.emulator.host}:{cosmos.bindings.emulator.port};DisableServerCertificateValidation=True;", property.Value.ValueExpression);
            });
    }
}
